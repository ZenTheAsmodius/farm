...
describe("UsersService", () => {
    ...
    describe("with existing user", () => {
      ...
      it("should throw UnprocessableEntityError if user already exists", async () => {
        await usersService.createUser(createUserDto).catch((error: UnprocessableEntityError) => {
          ...
          expect(error.message).toBe(`User with an email "${createUserDto.email}" is already registered.`);
        });
      });
    });
  });
  ...
  describe(".createUserV2", () => {
    const createUserDto: CreateUserDtoV2 = {
      email: "user@test.com",
      password: "password",
    ...
    it("should create new user", async () => {
      const createdUser = await usersService.createUserV2(createUserDto);
      expect(createdUser).toBeInstanceOf(User);
    });

    describe("with existing user", () => {
      beforeEach(async () => {
        await usersService.createUserV2(createUserDto);
      });
